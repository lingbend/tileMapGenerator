namespace NodeTreeGenerator
{
    ...
    public class NodeTreeGeneratorSettings
    {
        ...
        public static Func<Graph, ConcurrentDictionary<Vector2, Vertex>, Dictionary<int, int>, IEnumerable<(Vertex, int)>> RadialShaper{get;} = ((graph, backing, weight) =>
        {
            ...
            {
                vertices.Add(((int) Math.Abs(((Vector2) (vertex.Weight - middle)).Length()), vertex));
            }
        ...
        public static Func<Graph, ConcurrentDictionary<Vector2, Vertex>, Dictionary<int, int>, IEnumerable<(Vertex, int)>> CentralAxisXShaper{get;} = ((graph, backing, weight) =>
        {
            ...
            foreach (Vertex vertex in backing.Values.OrderBy(v=>v.ID))
            {
                vertices.Add(((int) Math.Abs((decimal) (vertex.Weight?.X - middle.X)), vertex));
            }
            vertices = new List<(int, Vertex)>(vertices.OrderBy((t)=>t.Item1));
        ...
        public static Func<Graph, ConcurrentDictionary<Vector2, Vertex>, Dictionary<int, int>, IEnumerable<(Vertex, int)>> CentralAxisYShaper{get;} = ((graph, backing, weight) =>
        {
            ...
            foreach (Vertex vertex in backing.Values.OrderBy(v=>v.ID))
            {
                vertices.Add(((int) Math.Abs((decimal) (vertex.Weight?.Y - middle.Y)), vertex));
            }
            vertices = new List<(int, Vertex)>(vertices.OrderBy((t)=>t.Item1));
        ...
        public static Func<Graph, ConcurrentDictionary<Vector2, Vertex>, (Graph, ConcurrentDictionary<Vector2, Vertex>)> HorizontalSymmetryPostProcessor{get;} = (graph, backing_dictionary) =>
        {
            ...
            if (weights.Count % 2 == 0)
            {
                ...
                median_list = weights.GetRange(0, ((weights.Count-1) / 2));
            ...
            else
            {
                ...
                var extra_vertex = new Vertex((Vector2) extra.Weight);
                new_backing_dictionary.TryAdd((Vector2) extra.Weight, extra_vertex);
            ...
            {
                Vertex new_vertex = new Vertex((Vector2) vertex.Weight);
                Vertex reflected_vertex = new Vertex(GetHorizontalReflection((Vector2) new_vertex.Weight, median));
                new_graph.AddVertex(new_vertex);
                new_graph.AddVertex(reflected_vertex);
                new_backing_dictionary.TryAdd((Vector2) new_vertex.Weight, new_vertex);
                new_backing_dictionary.TryAdd((Vector2) reflected_vertex.Weight, reflected_vertex);
            }
            if (extra != null)
            {
            ...
            foreach (Vertex vertex in median_list)
            {
                foreach (Edge edge in vertex.Edges.OrderBy(e=>e.ID))
                {
                    if (extra != null && vertex.Weight == extra?.Weight)
                    {
                        ...
                        if (!new_backing_dictionary.ContainsKey((Vector2) other_vertex.Weight))
                        {
                            continue;
                        }

                        Vector2 other_vertex_weight = (Vector2) other_vertex.Weight;
       
                        if (!new_graph.ContainsVertex(new_backing_dictionary[(Vector2) extra.Weight]))
                        {
                            throw new Exception();
                        }
                        ...
                        new_graph.AddEdge(new_backing_dictionary[(Vector2) extra.Weight]
                        .ConnectToVertex(new_backing_dictionary[other_vertex_weight], (Vector2) extra.Weight - other_vertex_weight));
                        new_graph.AddEdge(new_backing_dictionary[(Vector2) extra.Weight].ConnectToVertex(new_backing_dictionary
                        [GetHorizontalReflection(other_vertex_weight, median)], ((Vector2) extra.Weight - other_vertex_weight)*new Vector2(0, -1)));
                    }
                    else if(median_list.Contains(edge.Source) != median_list.Contains(edge.Target) && edge.Target != extra && edge.Source != extra)
                    {
                        ...
                        new_graph.AddEdge(new_backing_dictionary[(Vector2) insider.Weight].ConnectToVertex(new_backing_dictionary
                        [GetHorizontalReflection((Vector2) insider.Weight, median)], (Vector2) edge.Weight));
                        possible_connection_pairs.Add((insider, new_backing_dictionary
                        [GetHorizontalReflection((Vector2) insider.Weight, median)]));
                    }
                    else if (edge.Target != extra && edge.Source != extra)
                    {
                        new_graph.AddEdge(new_backing_dictionary[(Vector2) edge.Source.Weight]
                        .ConnectToVertex(new_backing_dictionary[(Vector2) edge.Target.Weight], (Vector2) edge.Weight));

                        new_graph.AddEdge(new_backing_dictionary[GetHorizontalReflection((Vector2) edge.Source.Weight, median)]
                        .ConnectToVertex(new_backing_dictionary[GetHorizontalReflection((Vector2) edge.Target.Weight, median)],
                        (Vector2) edge.Weight*new Vector2(0, -1)));
                    }
                }
            }
            ...
            while (connected_components.Count > 1 && counter > 0)
            {
                ...
                foreach (Graph subgraph in connected_components.Skip(1))
                {
                    ...
                    if (pairs_in_both.Count > 1)
                    {
                        ...
                        new_graph.AddVerticesAndEdge(pairs_in_both[0].Item1.ConnectToVertex(pairs_in_both[0].Item2, (Vector2) (pairs_in_both[0].Item2.Weight - pairs_in_both[0].Item1.Weight)));
                    ...
                    else if (connected_components.Count+1 > counter)
                    {
                        ...
                        new_graph.AddVerticesAndEdge(alternative_pairs[0].Item1.ConnectToVertex(alternative_pairs[0].Item2, (Vector2) (alternative_pairs[0].Item2.Weight - alternative_pairs[0].Item1.Weight)));
                        new_graph.AddVerticesAndEdgeRange(subgraph.Edges);
                    }
                }
        ...
        public static Func<Graph, ConcurrentDictionary<Vector2, Vertex>, (Graph, ConcurrentDictionary<Vector2, Vertex>)> VerticalSymmetryPostProcessor{get;} = (graph, backing_dictionary) =>
        {
            ...
            {
                Vertex new_vertex = new Vertex((Vector2) vertex.Weight);
                Vertex reflected_vertex = new Vertex(GetVerticalReflection((Vector2) new_vertex.Weight, median));
                new_graph.AddVertex(new_vertex);
                new_graph.AddVertex(reflected_vertex);
                new_backing_dictionary.TryAdd((Vector2) new_vertex.Weight, new_vertex);
                new_backing_dictionary.TryAdd((Vector2) reflected_vertex.Weight, reflected_vertex);
            ...
            foreach (Vertex vertex in median_list)
            {
                foreach (Edge edge in vertex.Edges.OrderBy(e=>e.ID))
                {
                    if (extra != null && vertex.Weight == extra?.Weight)
                    {
                        ...
                        .ConnectToVertex(new_backing_dictionary[other_vertex_weight], (Vector2) (extra.Weight - other_vertex_weight)));
                        new_graph.AddEdge(new_backing_dictionary[(Vector2) extra.Weight].ConnectToVertex(new_backing_dictionary
                        [GetVerticalReflection(other_vertex_weight, median)], (Vector2) (extra.Weight - other_vertex_weight)*new Vector2(-1, 0)));
                    }
                    else if(median_list.Contains(edge.Source) != median_list.Contains(edge.Target) && edge.Target != extra && edge.Source != extra)
                    {
                        Vertex insider = median_list.Contains(edge.Source) ? edge.Source : edge.Target;
                        new_graph.AddEdge(new_backing_dictionary[(Vector2) insider.Weight].ConnectToVertex(new_backing_dictionary
                        [GetVerticalReflection((Vector2) insider.Weight, median)], (Vector2) edge.Weight));
                        possible_connection_pairs.Add((insider, new_backing_dictionary
                        [GetVerticalReflection((Vector2) insider.Weight, median)]));
                    }
                    else if (edge.Target != extra && edge.Source != extra)
                    {
                        new_graph.AddEdge(new_backing_dictionary[(Vector2) edge.Source.Weight]
                        ...
                        new_graph.AddEdge(new_backing_dictionary[GetVerticalReflection((Vector2) edge.Source.Weight, median)]
                        .ConnectToVertex(new_backing_dictionary[GetVerticalReflection((Vector2) edge.Target.Weight, median)],
                        (Vector2) edge.Weight*new Vector2(-1, 0)));
                    }
                }
            }
            List<Graph> connected_components = new List<Graph>(NodeTreeGenerator.GetConnectedComponents(new_graph).OrderBy(g=>g.EdgeCount * g.VertexCount));
